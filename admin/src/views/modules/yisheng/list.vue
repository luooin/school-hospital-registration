<template>
  <div class="main-content">
        ...
        <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
            ...
            <el-table-column width="300" :align="contents.tableAlign" 
               :header-align="contents.tableAlign"
                ...
                <el-button v-if="isAuth('yisheng','医生信息') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 1" type="success" icon="el-icon-tickets" size="mini" @click="yishengxinxiCrossAddOrUpdateHandler(scope.row,'cross','','','')">{{ contents.tableBtnFont == 1?'医生信息':'' }}</el-button>
                <el-button v-if="isAuth('yisheng','医生信息') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 2" type="success" size="mini" @click="yishengxinxiCrossAddOrUpdateHandler(scope.row,'cross','','','')">{{ contents.tableBtnFont == 1?'医生信息':'' }}<i class="el-icon-tickets el-icon--right" /></el-button>
                <el-button v-if="isAuth('yisheng','医生信息') && contents.tableBtnIcon == 0" type="success" size="mini" @click="yishengxinxiCrossAddOrUpdateHandler(scope.row,'cross','','','')">{{ contents.tableBtnFont == 1?'医生信息':'' }}</el-button>
    ...
    <yishengxinxi-cross-add-or-update v-if="yishengxinxiCrossAddOrUpdateFlag" :parent="this" ref="yishengxinxiCrossaddOrUpdate"></yishengxinxi-cross-add-or-update>
...
import yishengxinxiCrossAddOrUpdate from "../yishengxinxi/add-or-update";
export default {
  data() {
    return {
      searchForm: {
      ...
      yishengxinxiCrossAddOrUpdateFlag: false,
  ...
  methods: {
    ...
    yishengxinxiCrossAddOrUpdateHandler(row,type,crossOptAudit,statusColumnName,tips,statusColumnValue){
      this.showFlag = false;
      this.addOrUpdateFlag = false;
      this.yishengxinxiCrossAddOrUpdateFlag = true;
		...
		for (var o in obj){
		  if(o==statusColumnName && obj[o]==statusColumnValue){
			...
			this.getDataList();
		      ...
		      this.showFlag = true;
		      this.yishengxinxiCrossAddOrUpdateFlag = false;
			return;
		  }
		}
	}
      this.$nextTick(() => {
      this.$refs.yishengxinxiCrossaddOrUpdate.init(row.id,type);
      });
    },
    init () {
    },
